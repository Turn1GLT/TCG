...
function fcnPostMatchResults(ss, RspnSht, ResponseData, EntryData, MatchID, OptDualSubmission, TestSht) {
  ...
  if (MatchValidWinr != 1){
    // returns Error that Winning Player match was illegal
    MatchPostedStatus = -2;
  }

  if (MatchValidWinr != 1){
    // returns Error that Losing Player match was illegal
    MatchPostedStatus = -3;
  }
