...
function subGetEmailAddress(ss, Addresses, WinPlyr, LosPlyr){
  ...
  var PlayerNames = shtPlayers.getRange(2,2,NbPlayers,1).getValues();
  
  // Find Players rows
  for (var row = 0; row < NbPlayers; row++){
    if (PlayerNames[row] == WinPlyr) rowWinr = row + 3;
    if (PlayerNames[row] == LosPlyr) rowLosr = row + 3;
    if (rowWinr > 0 && rowLosr > 0) row = NbPlayers + 1;
  }
  
  // Get Email addresses using the players rows
  Addresses[1] = shtPlayers.getRange(rowWinr,colEmail).getValue();
  Addresses[2] = shtPlayers.getRange(rowLosr,colEmail).getValue();
...
function fcnSendConfirmEmailEN(shtConfig, Address, MatchData) {
  ...
  var ssEmailID = shtConfig.getRange(37,2).getValue();
...
function fcnSendConfirmEmailFR(shtConfig, Address, MatchData) {
  ...
  // Set Email Subject
  EmailSubject = Location + ' ' + LeagueNameFR + " - Week " + Week + " - Match Result" ;
    
  // Start of Email Message
  EmailMessage = '<html><body>';
  
  EmailMessage += 'Hi ' + Winr + ' and ' + Losr + ',<br><br>Your match result has been received and succesfully processed for the ' + LeagueNameFR + ', Week ' + Week + 
    '<br><br>Here is your match result:<br><br>';
...
function fcnSendErrorEmailFR(shtConfig, Address, MatchData, MatchID, Status) {
  ...
  // Set Email Subject
  EmailSubject = Location + ' ' + LeagueNameFR + ' - Week ' + Week + ' - Match Report Error' ;
  ...
  // If Error prevented Match Data to be processed (Duplicate Entry or Player Match is not valid)  
  if (Status[0] < 0 && Status[0] > -60) {
    EmailMessage += 'Hi ' + Winr + ' and ' + Losr + ',<br><br>Your match result has been succesfully received for the ' + LeagueNameFR + ', Week ' + Week + 
      "<br><br>An error has been detected in one of the player's record. Unfortunately, this error prevented us to process the match report.<br><br>"+
        "<b>Error Message</b><br>" + StatusMsg +
          '<br><br>Here is your match result:<br><br>';
